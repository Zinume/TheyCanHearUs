...
func gravedadYAnimacion(delta):
	...
	if velocity.length() >0.2 and velocity.length() <2 and deboMoverme:
	...
	elif velocity.length() >2 and velocity.length() <10 and deboMoverme:
		velocity = velocity.normalized() * Speed
		#walk
		look_at(targ,Vector3.UP,true)
		rotation.x = 0
...
func distanciaConObjetivoMoverse(delta):
	if position.distance_to(targ)>1 and !estoyViendo and deboMoverme:
		...
		velocity = newVel
		if position.distance_to(targ)>4:
